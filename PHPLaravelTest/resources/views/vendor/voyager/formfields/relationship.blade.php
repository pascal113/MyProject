        ...
        @if($options->type == 'belongsTo')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @php
                    ...
                    $link = $relatedDataType && Auth::user()->hasPermission('read_'.$relatedDataType->name) && isset($query) ? route('voyager.' . $relatedDataType->slug.'.show', [ 'id' => $query->id ]) : null;
                @endphp
                    ...
                    @foreach($query as $relationshipData)
                        ...
                        $label = null;
                            if (old($options->column, $dataTypeContent->{$options->column}) == $relationshipData->{$options->key}) {
                                if ($relationshipField === 'order_belongsto_user_relationship' && ($dataTypeContent->user ?? null)) {
                                    $label = $dataTypeContent->user->first_name.' '.$dataTypeContent->user->last_name;
                                }
                            }
                        @endphp
                        <option value="{{ $relationshipData->{$options->key} }}" @if(old($options->column, $dataTypeContent->{$options->column}) == $relationshipData->{$options->key}) selected="selected" @endif>{{ $label ?? $relationshipData->{$options->label} }}</option>
                    @endforeach
                </select>
