...
@php
    $orderByExcludedColumns = $orderByExcludedColumns ?? [];
    $filters = $filters ?? [];
@endphp
...
@section('page_header')
    <div class="container-fluid">
        ...
        @can('delete', app($dataType->model_name))
            @can('bulk_delete', app($dataType->model_name))
        ...
        @if ($dataType->name === 'products')
            @can('read', app($dataType->model_name))
                <a href="{{ route('voyager.'.$dataType->slug.'.sync-wash-clubs') }}" class="btn btn-success btn-add-new">
                    <i class="voyager-refresh"></i> <span>Sync with WashConnect</span>
                </a>
            @endcan
        @endif
        @if ($dataType->name === 'orders')
            @can('read', app($dataType->model_name))
                <a href="{{ route('voyager.'.$dataType->slug.'.csv') }}" class="btn btn-primary btn-add-new">
                    ...
                    <div class="panel-body">
                        @if ($isServerSide)
                            <form method="get" class="form-search">
                                @if (count($filters))
                                    <div id="filters">
                                        @foreach ($filters as $filter)
                                            <div class="col-md-12">
                                                @include('voyager::bread.partials.filter')
                                            </div>
                                        @endforeach
                                    </div>
                                @endif

                                <div id="search-input" class="{{ count($filters) ? 'has-filters' : '' }}">
                                    ...
                                    @elseif (count($searchNames))
                                        <input type="hidden" name="key" value="{{ array_keys($searchNames)[0] }}">
                                ...
                                @endif
                                @if (Request::has('showSoftDeleted'))
                                    <input type="hidden" name="showSoftDeleted" value="{{ Request::get('showSoftDeleted') }}">
                                @endif
                            </form>
                        @endif
                            ...
                            <table id="dataTable" class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th>
                                            @if ($isServerSide && !in_array($row->field, $orderByExcludedColumns))
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            <td
                                                @if ($row->type == 'date' || $row->type == 'timestamp')
                                                    data-sort="{{ !$data->{$row->field} ? '0' : \Carbon\Carbon::parse($data->{$row->field})->timestamp }}"
                                                @endif                                            
                                            >
                                                ...
                                                @elseif($row->type == 'date' || $row->type == 'timestamp')
                                                    @php
                                                        $displayDate = 'never';
                                                        if ($data->{$row->field}) {
                                                            $displayDate = property_exists($row->details, 'format') ? \Carbon\Carbon::parse($data->{$row->field})->formatLocalized($row->details->format) : $data->{$row->field};
                                                        }
                                                    @endphp
                                                ...
                                                @elseif($row->type == 'media_picker')
                                                    ...
                                                    @else
                                                        {{ trans_choice('voyager::media.files', 0) }}
                                                    @endif
                                                @elseif($row->type == 'coupon_products')
                                                    @include('voyager::formfields.coupon_products', ['view' => 'browse', 'options' => $row->details])
                                                @elseif($row->type == 'order_products')
                                                    @include('voyager::formfields.order_products', ['view' => 'browse', 'options' => $row->details])
                                                @elseif($row->type == 'order_status')
                                                    @include('voyager::formfields.order_status', ['view' => 'browse', 'options' => $row->details])
                                                @elseif($row->type == 'order_customer')
                                                    @include('voyager::formfields.order_customer', ['view' => 'browse', 'options' => $row->details])
                                                @else
                                                    @include('voyager::multilingual.input-hidden-bread-browse')
                                                    @php
                                                        $moneyFields = [
                                                        ...
                                                        $prefix = in_array($row->field, $moneyFields) ? '$' : '';
                                                        $suffix = in_array($row->field, $percentageFields) ? '%' : '';
                                                        $content = in_array($row->field, $moneyFields) ? number_format($data->{$row->field}, 2) : $data->{$row->field};
                                                    @endphp
                        ...
                        @if ($isServerSide)
                            ...
                            <div class="pull-right">
                                {{ $dataTypeContent->appends([
                                    ...
                                    'filters' => collect($filters, [])->reduce(function($acc, $filter) {
